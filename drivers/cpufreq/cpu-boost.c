 ...
 */
static int
boost_adjust_notify (struct notifier_block *nb, unsigned long val, void *data)
{
...
static void
do_input_boost_rem (struct work_struct *work)
{
  unsigned int i;
...
_do_dsb ()
{
  ...
  ret = do_stune_boost ("top-app", dynamic_stune_boost_ta, &boost_slot_ta);
  if (!ret)
    stune_boost_active_ta = true;
  ret = do_stune_boost ("foreground", dynamic_stune_boost_fg, &boost_slot_fg);
  if (!err)
...
static void
do_dynamic_stune_boost_rem (struct work_struct *work)
{
...
static void
do_input_boost (struct work_struct *work)
{
  ...
  if (time_before
      (jiffies, last_ib_time + msecs_to_jiffies (input_boost_ms * 0.8)))
    return;
...
mdss_boost_kick ()
{
  if (mdss_boost_enabled < 1 || work_pending (&input_boost_work)
      || time_after (jiffies,
		     last_input_time + msecs_to_jiffies (mdss_timeout)))
    {
      return;
    }
...
static void
do_input_boost_max (unsigned int duration_ms)
{
  unsigned int i;
  struct cpu_sync *i_sync_info;
  cancel_delayed_work_sync (&input_boost_rem);
...
void
input_boost_max_kick (unsigned int duration_ms)
{
...
static void
cpuboost_input_event (struct input_handle *handle,
...
static int
cpuboost_input_connect (struct input_handler *handler,
...
static void
cpuboost_input_disconnect (struct input_handle *handle)
{
#ifdef CONFIG_DYNAMIC_STUNE_BOOST
  /* Reset dynamic stune boost value to the default value */
  reset_stune_boost ("top-app", boost_slot_ta);
